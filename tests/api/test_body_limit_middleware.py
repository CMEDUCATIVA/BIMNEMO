...
@pytest.mark.parametrize(
    "path,tier",
    [
        # BIMNEMO mirrors the upload per memory. Same operation, same body,
        # so it must get the same ceiling — without this it fell to the 1 MiB
        # default and an ordinary .docx was refused with a 413.
        ("/nemo/obra_sur/documents/upload", UPLOAD_LIMIT),
        ("/nemo/General/documents/upload", UPLOAD_LIMIT),
        # The pattern is anchored, not a suffix match: this module bounds the
        # unauthenticated surface, and a suffix match would hand the high
        # ceiling to any path ending in /documents/upload.
        ("/evil/documents/upload", DEFAULT_LIMIT),
        ("/nemo/a/b/documents/upload", DEFAULT_LIMIT),
        ("/nemo//documents/upload", DEFAULT_LIMIT),
        ("/nemo/x/documents/uploads", DEFAULT_LIMIT),
    ],
)
async def test_the_per_memory_upload_mirror_gets_the_upload_tier(path, tier):
    for declared, expected in ((tier, 200), (tier + 1, 413)):
        app = _BodyReader()
        recorder = _Recorder([b""])
        await _mw(app)(
            _scope(path=path, headers=[(b"content-length", str(declared).encode())]),
