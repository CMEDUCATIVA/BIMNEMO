...
from lightrag.api.routers.bimnemo_routes import create_bimnemo_routes
from lightrag.api.routers.nemo_routes import create_nemo_routes
from lightrag.api.bimnemo.nemo_manager import NemoManager
from lightrag.api.bimnemo.nemo_registry import NemoRegistry
...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            ...
            # BIMNEMO: hand the already-initialized default instance to the
            # NEMO manager instead of letting it build a second one for the
            # same workspace — two objects writing the same files. Pinned:
            # this instance's lifecycle belongs to THIS lifespan, so the
            # manager serves it but never evicts or closes it.
            nemo_manager.adopt(args.workspace, rag, pinned=True)
            ...
            # BIMNEMO: close every NEMO this manager opened, BEFORE the
            # default one below. The pinned default is skipped — it is closed
    ...
    def _build_rag(workspace: str) -> LightRAG:
        """Build a LightRAG bound to ``workspace``, with this server's config.

        Extracted into a factory so BIMNEMO's NEMO manager can build further
        instances for other workspaces from the SAME configuration. Copying
        this call site would guarantee the two versions drift apart.

        The default instance below is just its first caller, built with the
        server's configured workspace — behavior is unchanged.
        """
        return LightRAG(
    ...
    def build_nemo_rag(workspace: str) -> LightRAG:
        """Factory handed to BIMNEMO's NEMO manager.

        Builds the instance AND registers the role-LLM builder, so every NEMO
        gets the same runtime wiring as the default one. A NEMO missing that
        builder would fail the moment a role-specific model was reconfigured.
        """
        instance = _build_rag(workspace)
    ...
    try:
        rag = build_nemo_rag(args.workspace)
    except Exception as e:
    ...
    # BIMNEMO: registry of NEMOs (isolated memories) plus the manager that
    # keeps one LightRAG instance per NEMO. Purely additive — the default
    ...
    # one that answers.
    nemo_registry.ensure(args.workspace, default=True)
    nemo_manager = NemoManager(factory=build_nemo_rag)
    app.state.nemo_registry = nemo_registry
    app.state.nemo_manager = nemo_manager
    ...
    # BIMNEMO application surface (catalog, panel metrics, engine view and the
    # agent-facing memory facade). Purely additive: it reads what the engine
    # already stores and delegates writes to rag.ainsert.
    app.include_router(
        create_bimnemo_routes(rag, doc_manager, api_key, nemo_registry, nemo_manager)
    )
    # NEMO surface: manage the isolated memories, and reach each one by name.
    # The mirror routes under /nemo/{name}/… are how an external agent picks a
    # memory; the pre-existing /query and /documents/* keep their signatures
    # and keep answering for the default NEMO.
    app.include_router(
        create_nemo_routes(
            nemo_registry, nemo_manager, doc_manager, args.working_dir, api_key
        )
    )
