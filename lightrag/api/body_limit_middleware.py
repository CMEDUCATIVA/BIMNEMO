...
# BIMNEMO expone la misma subida por memoria: ``/nemo/<id>/documents/upload``.
# Es la MISMA operacion que ``/documents/upload`` — mismo cuerpo multipart,
# mismo fichero — asi que debe llevar el mismo techo. Sin esto le caia el
# limite general de 1 MiB y un .docx corriente se rechazaba con un 413.
#
# El patron es exacto y no un ``endswith``: este modulo acota la superficie sin
# autenticar, y una coincidencia por sufijo dejaria que cualquier ruta acabada
# en ``/documents/upload`` se llevara el techo alto.
NEMO_UPLOAD_RE = re.compile(r"^/nemo/[^/]+/documents/upload$")
...
class BodyLimitMiddleware:
    ...
    def _limit_for(self, scope: dict[str, Any]) -> int:
        ...
        if route == UPLOAD_PATH or NEMO_UPLOAD_RE.match(route):
